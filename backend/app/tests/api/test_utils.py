...
class TestUtils:
    ...
    def test_create_title(self):
        ...
        assert len(create_title(long_content)) == 20        
    
    def test_saveConversation_success(self):
        ...
        with patch('app.api.utils.create_conversation', return_value=mock_conversation) as mock_create:
            # Act
            result = saveConversation(db=self.mock_db, user_id=self.user_id, title=title)
            
            # Assert
            mock_create.assert_called_once()
            assert result.id == self.conversation_id
    
    def test_saveConversation_failure(self):
    ...
    def test_saveMessage_success(self):
        ...
        with patch('app.api.utils.create_message', return_value=mock_message) as mock_create:
            # Act
            result = saveMessage(
                db=self.mock_db, 
    ...
    def test_saveMessage_empty_content(self):
    ...
    def test_saveMessage_creation_fails(self):
    ...
    async def test_get_and_save_ai_response_success(self):
        ...
        with patch('app.api.utils.serialise_message_data', return_value={"content": "AI generated response"}) as mock_serialize:
            with patch('app.api.utils.saveMessage', return_value=mock_message) as mock_save:
                # Act
